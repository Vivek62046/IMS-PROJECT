...
class employeeClass:
    def __init__(self,root):
        ...
        btn_analysis = Button(self.root, text="Employee Analysis", command=self.employee_analysis, 
                      font=("goudy old style", 15, "bold"), bg="#ff5722", fg="white", cursor="hand2")
        btn_analysis.place(x=850, y=31, width=200, height=60)
...
# Employee Data 
  ...
  #   messagebox.showinfo("Employee Analysis", "This will show employee analysis in the future.")
    ...
    def employee_analysis(self):  
      ...
      try:
        ...
        if df.empty:
            messagebox.showinfo("Info", "No employee data to analyze.", parent=self.root)
            return
        ...
        plt.suptitle("Employee Data Analysis", fontsize=16)

        # 1. Gender Distribution Pie Chart
        plt.subplot(1, 2, 1)
        gender_counts.plot(kind='pie', autopct='%1.1f%%', startangle=90, colors=['skyblue', 'lightcoral', 'yellowgreen'])
        plt.title("Gender Distribution")
        plt.ylabel('')  # hide y-label

        # 2. Salary Distribution Histogram
        plt.subplot(1, 2, 2)
        df['salary'].dropna().plot(kind='hist', bins=10, color='mediumseagreen')
        ...
        messagebox.showerror("Error", f"Error during analysis: {str(ex)}", parent=self.root)
